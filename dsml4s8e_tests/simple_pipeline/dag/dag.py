...
def full_path(nb_name: str) -> str:
    p = Path(__file__)
    return str(p.parent.parent.joinpath('component', nb_name))


op_1_params = dagstermill_op_params_from_nb(full_path("nb_1.ipynb"))
op1 = define_dagstermill_op(**op_1_params,
                            save_notebook_on_failure=True)

op_2_params = dagstermill_op_params_from_nb(full_path("nb_2.ipynb"))
