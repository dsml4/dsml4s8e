...
def test_get_ins_data_paths():
    ...
    context = get_fake_context(run_id="0000", tags={"notebook_path": nb_path})
    op = NbOp(
        config_schema={"a": Field(int, description="nb params", default_value=10)},
    ...
    ins_data_urls: Dict[str, str] = op._get_ins_data_paths(locals_=locals())
    assert ins_data_urls["simple_pipeline.data_load.nb_1.data1"] == "path"
...
def test_get_data_paths():
    ...
    var_name = "val"
    urls = op.make_catalog(
        locals_=locals(),
        storage_catalog=LoacalStorageCatalog(
            "/home/jovyan/data", dagster_context=context
        ),
    )
    etalon_url = "/home/jovyan/data/dev/pipeline_example/data_load/0000/nb/data1"
    assert op._outs_dict["pipeline_example.data_load.nb.data1"] == etalon_url

    assert urls.outs.pipeline_example.data_load.nb.data1 == etalon_url
    assert urls.ins.pipeline_example.data == "val"
